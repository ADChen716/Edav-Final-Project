...
write.csv(global_daily_df_cleaned,"shiny/www/daily_global.csv")
...
write.csv(global_daily_df_cleaned,"shiny/www/yearly_global.csv")

# without 2 id
daily_df_cleaned <- daily_df[,c(2:7,10:11,13,15:21)]
global_daily_df_cleaned <- daily_df_cleaned[which(daily_df_cleaned$Region=="global"),]
write.csv(global_daily_df_cleaned,"shiny/www/daily_global_no_id.csv")

yearly_df_cleaned <- yearly_df[,c(1:5,9,10,12,14:20)]
global_yearly_df_cleaned <- yearly_df_cleaned[which(yearly_df_cleaned$Region=="global"),]
write.csv(global_daily_df_cleaned,"shiny/www/yearly_global_no_id.csv")
...
write.csv(plot_df,"shiny/www/choropleth_cleaned.csv")
